...
function CopyChip({ icon, value, href, copyText }: { icon: ReactNode; value: string; href: string; copyText?: string }) {
  ...
  return (
    <a
      ...
      onClick={handleClick}
      className="group relative flex items-center gap-3 bg-[#fee95a] hover:bg-yellow-300 transition-all px-5 py-3 rounded-full shadow-md hover:shadow-xl hover:scale-105 active:scale-95"
    >
      <span className="text-[#016634] text-[20px]">{icon}</span>
      <span
        className="text-[#016634] text-[14px] sm:text-[15px]"
